...
// 会导致编译错误，这里只写的定义方式为`chan<- string`表示这个类型为 字符串的通道为只写通道
...
func main() {
    ...
    pong(pings, pongs)
    fmt.Println(<-pongs)
    // 其实这个例子就是把信息首先写入pings通道里面，然后在pong函数里面再把信息从pings通道里面读出来再写入pongs通道里面，
    // 最后在main函数里面将信息从pongs通道里面读出来。
