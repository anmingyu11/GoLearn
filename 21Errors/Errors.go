...
// 这个和Java或者Ruby里面使用异常或者是C里面运行正常返回结果，
// 发生错误返回错误代码的方式不同。Go的这种错误处理的方式使得我们能够很容易看出哪些函数可能返回错误，并且能够像调用那些没有错误返回的函数一样调用。
...
func f1(arg int) (int, error) {
    if arg == 42 {
        ...
        return -1, errors.New("Cannot work")
    }
...
func main() {
    ...
    // 如果你需要使用自定义错误类型返回的错误数据，你需要使用类型断言 来获得一个自定义错误类型的实例才行。
    _, e := f2(42)
